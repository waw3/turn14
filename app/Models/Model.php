...
use App\Enums\Webhook\Method;
use App\Exceptions\AuthenticationException;
use App\Exceptions\InvalidParamsException;
use App\Exceptions\InvalidWebhookMethodException;
use App\Exceptions\WebhookSecretMissingException;
...
abstract class Model implements Arrayable, ArrayAccess
{
    ...
    {
        if (!config('turn14.webhook_secret')) {
        ...
        $url = route('handle-turn14-webhook', $routeParameters);
        ...
        $client = Http::withOptions([
            ...
            'Client-ID' => config('turn14.credentials.client_id'),
        ...
        $response = $client->post($endpoint, [
            'url' => $url,
            'method' => $parsedMethod,
            'secret' => config('turn14.webhook_secret'),
        ])->throw()->json();
