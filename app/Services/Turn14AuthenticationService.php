...
class Turn14AuthenticationService
{
    use ConsumesExternalServices, InteractsWithTurn14Responses;
    ...
    public function __construct()
    {
        ...
        $this->clientId = config('services.turn14.client_id');
        $this->clientSecret = config('services.turn14.client_secret');
        $this->passwordClientId = config('services.turn14.password_client_id');
        $this->passwordClientSecret = config('services.turn14.password_client_secret');
    }
...
//         ];
//
//         $tokenData = $this->makeRequest('POST', 'token', [], $formParams);

        $tokenData = Http::turn14()->withBasicAuth(
                        $this->clientId,
                        $this->clientSecret,
                    )->asForm()->post(
                        'token',
    ...
    public function resolveAuthorizationUrl()
    {
        ...
        return "{$this->baseUri}/authorize?{$query}";
    }
